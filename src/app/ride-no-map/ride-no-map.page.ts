...
export class RideNoMapPage implements OnInit, AfterViewInit, OnDestroy, ViewDidEnter, ViewWillEnter {
  ...
  loadUpcomingRide() {
    console.log('RideNoMapPage: loadUpcomingRide() called - fetching pending bookings');
    ...
    console.log('RideNoMapPage: Making API call to /app/bookings with rick:', rick);
    ...
    this.http.post<{ success?: boolean; message?: string; data?: Booking[] } | Booking[]>(`${this.apiUrl}/app/bookings`, requestBody)
      ...
      .subscribe({
        next: (response) => {
          console.log('RideNoMapPage: API response received', response);
          ...
          if (pendingBookings.length > 0) {
            ...
            console.log('RideNoMapPage: Found pending ride:', this.upcomingRide);
            ...
            console.log('RideNoMapPage: No pending bookings found');
            this.upcomingRide = null;
            this.isRideAccepted.set(false);
          }
  ...
  getOnlineStatus() {
    ...
    if (!driverId) {
      console.error('RideNoMapPage: Driver ID not found. Cannot fetch online status.');
      return;
    }
    ...
    this.http.post<{ success: boolean; data: { id: number; online: boolean }; message: string }>(`${this.apiUrl}/app/drivers/get-online-status`, requestBody)
      ...
      .subscribe({
        next: (response) => {
          console.log('RideNoMapPage: Online status received', response);
          ...
          if (response && response.success && response.data && typeof response.data.online === 'boolean') {
            ...
            console.warn('RideNoMapPage: Unexpected response format for online status', response);
          }
          
          this.isOnline = onlineStatus;
          console.log('RideNoMapPage: Updated online status to:', this.isOnline);
        ...
        error: (error) => {
          console.error('RideNoMapPage: Failed to get online status:', error);
  ...
  toggleOnlineStatus() {
    ...
    if (!driverId) {
      console.error('RideNoMapPage: Driver ID not found. Please login again.');
    ...
    this.http.post<{ success?: boolean; message?: string }>(`${this.apiUrl}/app/drivers/update-online`, requestBody)
      .pipe(
        catchError((error: HttpErrorResponse | Error) => {
          ...
          console.error('RideNoMapPage: Update online status API Error:', errorMessage);
      ...
      .subscribe({
        next: (response) => {
          console.log('RideNoMapPage: Online status updated successfully', response);
        ...
        error: (error) => {
          console.error('RideNoMapPage: Failed to update online status:', error);
    ...
    this.http.post<{ success?: boolean; message?: string }>(`${this.apiUrl}/app/bookings/update-status`, requestBody)
      .pipe(
        catchError((error: HttpErrorResponse | Error) => {
          ...
          console.error('RideNoMapPage: Accept ride API Error:', errorMessage);
      ...
      .subscribe({
        next: (response) => {
          console.log('RideNoMapPage: Ride accepted successfully', response);
          this.isUpdatingStatus = false;
        ...
        error: (error) => {
          console.error('RideNoMapPage: Failed to accept ride:', error);
          this.isUpdatingStatus = false;
    ...
    this.http.post<{ success?: boolean; message?: string }>(`${this.apiUrl}/app/bookings/update-status`, requestBody)
      .pipe(
        catchError((error: HttpErrorResponse | Error) => {
          ...
          console.error('RideNoMapPage: Reject ride API Error:', errorMessage);
          this.isUpdatingStatus = false;
      ...
      .subscribe({
        next: (response) => {
          console.log('RideNoMapPage: Ride rejected successfully', response);
          this.isUpdatingStatus = false;
          ...
          setTimeout(() => {
            console.log('RideNoMapPage: Loading next pending ride after rejection');
            this.loadUpcomingRide();
          }, 300);
        },
        error: (error) => {
          console.error('RideNoMapPage: Failed to reject ride:', error);
          this.isUpdatingStatus = false;
          // Error already handled in catchError, but keep this for additional handling if needed
        }
      });
